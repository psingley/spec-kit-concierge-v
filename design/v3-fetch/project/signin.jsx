...
function SignInScreen({ auth, setAuth }) {
  ...
  const atlStarting = auth.atlassian === "starting";
    ...
    <div className="signin-stage">
      <div className="signin-card">
        ...
        <div className="signin-rows">
          ...
          <div className={"signin-row " + (atlOk ? "is-on" : "")}>
            <div className="signin-row-icon"><Ico.Atlassian size={15} /></div>
            <div className="signin-row-main">
              <div className="signin-row-title">Atlassian MCP</div>
              <div className="signin-row-sub">
                {atlOk ? "Connected to collette-travel.atlassian.net" : "Required to send specs to JIRA"}
              </div>
            </div>
            {atlOk ? (
              <span className="signin-row-status">
              ...
              <button
                className="btn primary"
                onClick={() => setAuth(a => ({ ...a, atlassian: "ok" }))}
                disabled={atlStarting}
              >
                <Ico.Atlassian size={12} />
                {atlStarting ? "Connecting…" : "Sign in"}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="signin-foot">
          <span>Trouble signing in? Use the gear menu to report a bug.</span>
        </div>
      </div>
    </div>
  );
