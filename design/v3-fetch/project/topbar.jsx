...
// --- Gear menu ---
function GearMenu({ log, onAbout, onFileRequest, onCustomize }) {
  const [open, setOpen] = React.useState(false);
  const ref = useClickOutside(open, () => setOpen(false));
    ...
    <div className="tb-chip-wrap" ref={ref}>
      ...
      {open && (
        <div className="tb-menu gear-menu">
          <button className="gear-item" onClick={() => { setOpen(false); onCustomize(); }}>
            <Ico.Gear size={13} />
            <span>Customize</span>
          </button>
...
function AuthChip({ auth, setAuth }) {
  ...
  const atlOk = auth.atlassian === "ok";
  // All three are required to enter the workspace.
  const allOk = ghOk && copOk && atlOk;
  const okCount = (ghOk ? 1 : 0) + (copOk ? 1 : 0) + (atlOk ? 1 : 0);

  const status = allOk ? "ok" : okCount === 0 ? "off" : "partial";
  const label = allOk
    ? "a.kim"
    : okCount > 0
      ? `${okCount} of 3`
  ...
  return (
    <div className="tb-chip-wrap" ref={ref}>
      <button
        ...
        onClick={() => setOpen(v => !v)}
        title={
          allOk ? "Authenticated · click to manage"
          : okCount > 0 ? "Partial sign-in · click to finish"
          : "Sign in to continue"
        }
      >
      ...
      {open && (
        <div className="tb-menu auth-menu">
          ...
          <div className="auth-menu-body">
            ...
            />
            <AuthMiniRow
              icon={<Ico.Atlassian size={12} />}
              label="Atlassian MCP"
              sub={atlOk ? "collette-travel.atlassian.net" : "required for JIRA sync"}
              state={auth.atlassian}
              onToggle={() => setAuth(a => ({ ...a, atlassian: a.atlassian === "ok" ? "out" : "ok" }))}
            />
          </div>
        </div>
      )}
