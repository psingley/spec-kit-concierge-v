...
export const createRun6BoundaryFixture = async (): Promise<BoundaryFixture> => {
  ...
  const featureDirectory = 'specs/0001-hello-world-feature';
  await writeFile(path.join(repoPath, 'README.md'), '# Run 6 fixture\n', 'utf8');
  await mkdir(path.join(repoPath, '.specify'), { recursive: true });
  await writeFile(
    path.join(repoPath, '.specify', 'feature.json'),
    `${JSON.stringify({ feature_directory: featureDirectory }, null, 2)}\n`,
    'utf8'
  );
  await execFileAsync('git', ['add', 'README.md', '.specify/feature.json'], { cwd: repoPath });
...
export const gitLogLastMessage = async (repoPath: string): Promise<string> => {
  const { stdout } = await execFileAsync('git', ['log', '--all', '-1', '--format=%B'], { cwd: repoPath });
  return stdout;
};
