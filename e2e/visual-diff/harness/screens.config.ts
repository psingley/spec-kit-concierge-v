...
const renderPassiveState = async (
  ...
  const modalMarkup = modal ? `
    <div class="modal-veil" data-vd-role="modal-veil">
      <div role="dialog" aria-modal="true" aria-labelledby="artifact-viewer-title" class="modal artifact-viewer">
        <div class="modal-head"><span>▣</span><h2 id="artifact-viewer-title">${passiveArtifacts[step][0]}</h2><button type="button" class="icon-btn" aria-label="Close artifact viewer">×</button></div>
        <div class="modal-body artifact-viewer-body"><div class="md-preview"><article class="markdown"><h1>${label} Artifact</h1><table><thead><tr><th>File</th><th>Status</th></tr></thead><tbody><tr><td>${passiveArtifacts[step][0]}</td><td>Validated</td></tr></tbody></table></article></div></div>
      </div>
    </div>
  ` : '';
  await reachWorkspace(page);
  const fixtureStyles = `<style>
    ...
    .modal-veil{position:fixed;inset:0;z-index:100;display:grid;place-items:center;padding:40px;background:rgb(0 0 0 / .5);backdrop-filter:blur(2px)}
    .modal{width:min(860px,calc(100vw - 40px));max-height:min(760px,calc(100vh - 80px));overflow:hidden;padding:0;background:var(--bg-2);border:1px solid var(--line-strong);border-radius:var(--radius-lg);box-shadow:var(--shadow-2)}
    .modal-head{display:flex;align-items:center;gap:10px;padding:16px 20px;border-bottom:1px solid var(--line)}.modal-head h2{margin:0;font-size:14px;font-weight:500;flex:1}.icon-btn{width:32px;height:32px;border:1px solid var(--line-strong);border-radius:8px;background:var(--surface-2);color:var(--text)}.modal-body{padding:18px 20px;overflow:auto;min-height:min(420px,calc(100vh - 220px));max-height:calc(100vh - 160px)}
  </style>`;
  await page.evaluate(`
    {
      const main = document.querySelector('main');
...
const renderReviewState = async (
  ...
  const taskModal = state === 'task-modal'
    ? '<div class="modal-veil" data-vd-role="modal-veil"><div class="modal artifact-viewer" role="dialog" aria-modal="true" aria-labelledby="artifact-viewer-title"><div class="modal-head"><span>▣</span><h2 id="artifact-viewer-title">tasks.md</h2><button type="button" class="icon-btn" aria-label="Close artifact viewer">×</button></div><div class="modal-body artifact-viewer-body"><div class="task-viewer"><div class="task-row"><strong>T001</strong><span>Build review:evidence aggregator</span><span class="meta">src/main/domain/reviewEvidence.ts</span></div></div></div></div></div>'
    : '';
...
export const screens: VisualDiffScreen[] = [
  ...
  { name: 'passive-artifact-modal-1280x800', designPath: 'design/v3-fetch/project/steps.jsx', designSetup: (page) => renderPassiveState(page, 'plan', 'done', true), shippedSetup: (page) => renderPassiveShippedState(page, 'plan', 'done', true), viewport: { width: 1280, height: 800 }, masks: [bodyTextMask, timestampMask, scrollbarMask] },
  { name: 'passive-artifact-modal-1440x900', designPath: 'design/v3-fetch/project/steps.jsx', designSetup: (page) => renderPassiveState(page, 'plan', 'done', true), shippedSetup: (page) => renderPassiveShippedState(page, 'plan', 'done', true), viewport: { width: 1440, height: 900 }, masks: [bodyTextMask, timestampMask, scrollbarMask] },
  ...
  { name: 'review-task-modal-1280x800', designPath: 'design/v3-fetch/project/steps.jsx', designSetup: (page) => renderReviewState(page, 'task-modal'), shippedSetup: (page) => renderReviewState(page, 'task-modal'), viewport: { width: 1280, height: 800 }, masks: [bodyTextMask, scrollbarMask] },
  { name: 'review-task-modal-1440x900', designPath: 'design/v3-fetch/project/steps.jsx', designSetup: (page) => renderReviewState(page, 'task-modal'), shippedSetup: (page) => renderReviewState(page, 'task-modal'), viewport: { width: 1440, height: 900 }, masks: [bodyTextMask, scrollbarMask] },
